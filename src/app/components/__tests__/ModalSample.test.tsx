...
jest.mock('../common/button', () => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return ({ children, onClick, variant, type, ...props }: any) => (
    <button
      onClick={onClick}
...
describe('ModalSample', () => {
  ...
  it('全てのセクション見出しが表示される', () => {
    ...
    expect(screen.getByRole('heading', { name: '基本モーダル' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '確認モーダル' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'フォームモーダル' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '大サイズモーダル' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'タイトルなしモーダル' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Modalコンポーネントの特徴' })).toBeInTheDocument();
  });
  ...
  describe('基本モーダル', () => {
    it('基本モーダルを開閉できる', async () => {
      ...
      expect(screen.getByTestId('modal-title')).toHaveTextContent('基本モーダル');
      expect(screen.getByTestId('modal')).toHaveAttribute('data-size', 'md');
      
      // モーダル内容を確認
      expect(screen.getByText('これは基本的なモーダルコンポーネントのサンプルです。')).toBeInTheDocument();
  ...
  describe('フォームモーダル', () => {
    it('フォームモーダルを開閉できる', async () => {
      ...
      // モーダルが開かれる
      expect(screen.getByTestId('modal')).toBeInTheDocument();
      expect(screen.getByTestId('modal-title')).toHaveTextContent('お問い合わせフォーム');
  ...
  describe('大サイズモーダル', () => {
    it('大サイズモーダルを開閉できる', async () => {
      ...
      // モーダル内容を確認
      expect(screen.getByText('これは大サイズのモーダルです。より多くのコンテンツを表示できます。')).toBeInTheDocument();
      expect(screen.getByText('モーダルのサイズバリエーション')).toBeInTheDocument();
  ...
  describe('Modalコンポーネントの特徴セクション', () => {
    it('特徴セクションが表示される', () => {
      ...
      // 機能リストを確認
      expect(screen.getByText('• 複数のサイズバリエーション')).toBeInTheDocument();
